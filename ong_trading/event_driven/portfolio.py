...
class NaivePortfolio(Portfolio):
    ...
    def generate_naive_order(self, signal: SignalEvent):
        ...
        desidered_position = mkt_quantity * direction.value
        order_quantity = desidered_position - cur_position

        if self.instrument == InstrumentType.Stock:
            # Logic for stocks: position can be only either long or flat
            if direction == DirectionType.SELL and cur_position > 0:
                # Stocks cannot go short, so never sell more than current position
                order = OrderEvent(symbol, limit_price, min(cur_position, mkt_quantity),
                                   direction, instrument=self.instrument)
            elif direction == DirectionType.BUY and cur_position < mkt_quantity:
                # Go longer if requested
                order = OrderEvent(symbol, limit_price, mkt_quantity - cur_position, direction,
                                   instrument=self.instrument)
            return order
        elif self.instrument == InstrumentType.CfD:
            # Logic for CFD: position can be either long or short
            if direction == DirectionType.SELL and cur_position >= 0:  # Go short only if position was flat or long, don't go shorter
                order = OrderEvent(symbol, limit_price, mkt_quantity + cur_position, direction, instrument=self.instrument)
            elif direction == DirectionType.BUY and cur_position <= 0:  # go long only if position was flat or short, don't go longer
                order = OrderEvent(symbol, limit_price, mkt_quantity - cur_position, direction, instrument=self.instrument)
            return order
    ...
    def output_summary_stats(self, plot=True):
        """
        Creates a list of summary statistics for the portfolio such
        as Sharpe Ratio and drawdown information.
